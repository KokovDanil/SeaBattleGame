...
public class Player {
    ...
    boolean shooting(Field field, boolean randomShot, boolean isOptimal) {
        ...
        if (randomShot) {
            ...
            do {
                error = false;
                System.out.println("Ваш выстрел, " + this.name + ". Введите координату (пример ввода: d4)");
                String s = (new Scanner(System.in)).nextLine();
                x = s.charAt(0) - 97;
                y = s.charAt(1) - 49;
                if (s.length() > 2) y = (s.charAt(1) - 48) * 10 + s.charAt(2) - 49;
                if (x < 0 || x > 9 || y < 0 || y > 9) error = true;
                if (error) System.out.println("Значение за пределами поля! Попробуйте еще раз.");
            } while (error);
        }
    ...
    Point searchFourDeck() {
        ...
        searchPoint.setX(count % 4);
        searchPoint.setY(count % 4);
        if (count / 4 == 0) {
            searchPoint.setY(searchPoint.getY() + 6);
        }
        if (count / 4 == 1) {
            searchPoint.setX(searchPoint.getX() + 4);
            searchPoint.setY(searchPoint.getY() + 6);
        }
        if (count / 4 == 2) {
            searchPoint.setX(searchPoint.getX() + 6);
            searchPoint.setY(searchPoint.getY() + 4);
        }
        if (count / 4 == 3) {
            searchPoint.setY(searchPoint.getY() + 2);
        }
        if (count / 4 == 4) {
            searchPoint.setX(searchPoint.getX() + 2);
        }
        if (count / 4 == 5) {
            searchPoint.setX(searchPoint.getX() + 6);
        }
    ...
    Point searchTwoDeck() {
        ...
        searchPoint.setX(i);
        int j = count50 % 5;
        if (i % 2 == 0) searchPoint.setY(j * 2);
        else searchPoint.setY(j * 2 + 1);
        count50++;
        return searchPoint;
    }
