    ...
    void manualPlacement(Player player) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            do {
                ...
                if (!player.field.checkFreeSpace(firstPoint.getX(), firstPoint.getY(), deckQty, isHorizontal, player.field)) {
                    gameWindow.textOut("Здесь нельзя располагать корабль. Укажите местоположение " + (i + 1) + "-го 1-палубного корабля");
                }
            } while (!player.field.checkFreeSpace(firstPoint.getX(), firstPoint.getY(), deckQty, isHorizontal, player.field));
            player.ships[0][i] = new Ship(player1.field.cells[firstPoint.getX()][firstPoint.getY()], 1, isHorizontal, player.field);
            gameWindow.update(textView.fieldToColor(player1, true), 1);
            gameWindow.repaint();
        }
        for (int i = 1; i < 4; i++) {
            for (int j = 0; j < player.ships[i].length; j++) {
                ...
                do {
                    ...
                    if (firstPoint.getY() == secondPoint.getY()) isHorizontal = true;
                    else isHorizontal = false;
                    if (isHorizontal && secondPoint.getX() < firstPoint.getX() || !isHorizontal && secondPoint.getY() < firstPoint.getY())
                ...
                player.ships[i][j] = new Ship(player1.field.cells[firstPoint.getX()][firstPoint.getY()], deckQty, isHorizontal, player.field);
                gameWindow.update(textView.fieldToColor(player1, true), 1);
                gameWindow.repaint();
            }
        }
